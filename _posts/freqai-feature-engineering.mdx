...
After defining the base features, you can expand upon them using the `feature_parameters` in the configuration file. This allows you to include additional timeframes, correlated pairs, and recent candles. Here’s an example configuration:
...
```mermaid
graph TD
    A[Start] --> B[Define Base Features]
    B --> C[Expand Features]
    C --> D[Customize Pipeline]
    D --> E[Detect Outliers]
    E --> F[Reduce Dimensionality]
    F --> G[Train Model]
    G --> H[Make Predictions]
    H --> I[End]

    C --> J{feature_engineering_expand_* functions}
    J --> K[feature_engineering_expand_all]
    J --> L[feature_engineering_expand_basic]
    J --> M[feature_engineering_standard]
    J --> N[set_freqai_targets]

    D --> O{Pipeline Customization}
    O --> P[use_SVM_to_remove_outliers]
    O --> Q[principal_component_analysis]

    E --> R[Dissimilarity Index (DI)]
    E --> S[Support Vector Machine (SVM)]
    E --> T[DBSCAN]
